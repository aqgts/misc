...
describe("Plane", () => {
  it("through", () => {
    const expectedPlane = new Plane(0, 1, 0, -1);
    const actualPlane = Plane.through(new Vector3(0, 1, 0), new Vector3(0, 1, 1), new Vector3(1, 1, 0));
    expect(actualPlane.equals(expectedPlane)).toBe(true);
  });
  it("intersection", () => {
    const plane1 = new Plane(0, 1, 0, -1);
    const plane2 = new Plane(0, 0, 1, -1);
    const expectedIntersections = [
      new Line3D(new Vector3(0, 1, 1), new Vector3(1, 0, 0))
    ];

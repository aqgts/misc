...
export default class InternalPMX {
  constructor(header, info, vertices, faces, textures, materials, bones, morphs, displayElementGroups, rigidBodies, joints) {
    this.header = header;
    this.info = info;
  ...
  write() {
    const io = new InternalPMX.SequentialAccessBinary(new SequentialAccessBinary(), this.header);
  ...
  static read(binary) {
    ...
    return new this(header, info, vertices, faces, textures, materials, bones, morphs, displayElementGroups, rigidBodies, joints);
  }
}
InternalPMX.Header = class Header {
...
InternalPMX.SequentialAccessBinary = class SequentialAccessBinary extends SequentialAccessBinary {
...
InternalPMX.Vertex = class Vertex {
...
InternalPMX.Texture = class Texture {
...
InternalPMX.Material = class Material {
...
InternalPMX.Bone = class Bone {
  constructor(japaneseName, englishName, position, parentIndex, deformationOrder, connection, isRotatable, isMovable, isVisible, isControllable, ikInfo, localAdditionMode, addsRotation, addsDisplacement, addition, fixedAxis, localAxis, deformsAfterPhysics, keyValue) {
    this.japaneseName = japaneseName;
    this.englishName = englishName;
    this.position = position;
    ...
    this.addsRotation = addsRotation;
    this.addsDisplacement = addsDisplacement;
  ...
  clone() {
    return new this.constructor(
      ...
      this.hasAddition() ? {parentIndex: this.addition.parentIndex, rate: this.addition.rate} : null,
  ...
  write(io) {
    ...
    if (this.hasAddition()) {
      io.writeBoneIndex(this.addition.parentIndex);
      io.writeFloat32(this.addition.rate);
    }
    if (this.fixesAxis()) {
  ...
  static read(io) {
    ...
    return new this(japaneseName, englishName, position, parentIndex, deformationOrder, connection, isRotatable, isMovable, isVisible, isControllable, ikInfo, localAdditionMode, addsRotation, addsDisplacement, addition, fixedAxis, localAxis, deformsAfterPhysics, keyValue);
  }
};
InternalPMX.Bone.IKInfo = class IKInfo {
...
InternalPMX.Bone.IKInfo.Link = class Link {
...
InternalPMX.DisplayElementGroup = class DisplayElementGroup {
...
InternalPMX.DisplayElementGroup.DisplayElement = class DisplayElement {
...
InternalPMX.RigidBody = class RigidBody {
...
InternalPMX.Joint.ConcreteJoint = {

...
new Vue({
  ...
  methods: {
    ...
    async createModel() {
      ...
      try {
        log.clear();
        const binary = await Core.run(await BinaryUtils.readBinaryFromFileAsync(modelFile), Number(this.loopCount), this.targetMaterialIndices.split(/, */).map(Number), log);
